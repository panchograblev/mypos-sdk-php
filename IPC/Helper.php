...
class Helper
{
    ...
    {
        return in_array($outputFormat, array(Defines::COMMUNICATION_FORMAT_XML, Defines::COMMUNICATION_FORMAT_JSON));
    }
    ...
    public static function isValidCardNumber($cardNo)
    {
        ...
        if ((!is_numeric($cardNo)) || (strlen($cardNo) > 19) || (strlen($cardNo) < 13)) return false;
        $sum = $dub = $add = $chk = 0;
        $even = 0;
     ...
     * @return array
     */
    public static function getValuesFromMultiDemensionalArray($array, $values = array())
    {
        if (!is_array($array)) {
            return $values;
        }
